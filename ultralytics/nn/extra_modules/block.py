...
class MutilScaleEdgeAugmenter(nn.Module):
    def __init__(self, inc, bins):
        ...
        self.ees = []
        for _ in bins:
            self.ees.append(EdgeAugmenter(inc // len(bins)))
...
class MultiScaleEdgeAugmentBlock(nn.Module):
    def __init__(self, inc, bins):
        super().__init__()
...
class C3k_MultiScaleEdgeAugmentBlock(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(MutilScaleEdgeAugmenter(c_, [3, 6, 9, 12]) for _ in range(n)))

class MultiScaleEdgeAugmentNet(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_MultiScaleEdgeAugmentBlock(self.c, self.c, 2, shortcut, g) if c3k else MutilScaleEdgeAugmenter(self.c, [3, 6, 9, 12]) for _ in range(n))
